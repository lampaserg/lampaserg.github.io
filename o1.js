(function() {
    ...
    var SERVER_CONFIG = {
        s1: {
            ...
            token: decodeHidden('ZjgzNzcwNTctOTBlYi00ZDc2LTkzYzktNzYwNTk1MmEwOTZs'),
            tokenParam: decodeHidden('c2hvd3lfdG9rZW4='),
            getHost: function() { return this.mirrors[this.currentIndex] || this.mirrors[0]; },
            getSubtitle: function() { return this.label; },
            auth: function(url, cfg) {
                if (url.indexOf('uid=') === -1)
                    url = Lampa.Utils.addUrlComponent(url, 'uid=' + cfg.uid);
                if (url.indexOf(cfg.tokenParam + '=') === -1)
                    url = Lampa.Utils.addUrlComponent(url, cfg.tokenParam + '=' + encodeURIComponent(cfg.token));
                else
                    url = url.replace(new RegExp(cfg.tokenParam + '=([^&]+)'), cfg.tokenParam + '=' + encodeURIComponent(cfg.token));
                return url;
            }
        }
    };
    ...
    function resetConnectionSourceState(source) {
        var cfg = SERVER_CONFIG[source];
        if (!cfg) return;
        if (typeof cfg.currentIndex !== 'undefined') cfg.currentIndex = 0;
    }
    ...
    function getHost() {
        ...
        return SERVER_CONFIG.s1.getHost();
    }
    ...
    var unic_id = SERVER_CONFIG.s1.uid;
    ...
    var rchBase = getHost().replace(/\/$/, '');
    var hostkey = rchBase.replace('http://', '').replace('https://', '');
    ...
    window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
        if (!window.rch_nws[hostkey].startTypeInvoke) {
            ...
            if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) check(true);
                ...
                net.silent(rchBase.indexOf(location.host) >= 0 ? 'https://github.com/' : host + '/cors/check', function() {
                    check(true);
                }, function() {
                    check(false);
                }, false, {
    ...
    window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
        window.rch_nws[hostkey].typeInvoke(rchBase, function() {
    ...
    function rchRun(json, call) {
        if (typeof NativeWsClient == 'undefined') {
            Lampa.Utils.putScript([rchBase + "/js/nws-client-es5.js?v18112025"], function() {}, false, function() {
                rchInvoke(json, call);
            }, true);
        } else {
            rchInvoke(json, call);
    ...
    function component(object) {
        ...
        this.switchConnectionSource = function(er) { this.noConnectToServer(er); };
    ...
    function startPlugin() {
        ...
        var manifst = {
            ...
            onContextLauch: function onContextLauch(object) {
                resetTemplates();
                Lampa.Component.add('sh_online', component);
    ...
    if (!window.__sh_runtime) startPlugin();
