(function () {
    ...
    function getVoiceWeightByName(name) {
    ...
    function getSourceWeightByName(source) {
    ...
    function logAnalysisResult(movie, allItems) {
        if (!DEBUG.logToConsole) return;
        ...
        console.log('%c  Фильм: ' + (movie.title || movie.name || 'unknown'), 'color: #4fc3f7;');
        console.log('%c═══════════════════════════════════════════════════════════', 'color: #4fc3f7; font-weight: bold; font-size: 14px;');
        console.log('');
        ...
        allItems.forEach(function(item) {
            ...
            if (!grouped[source]) {
                grouped[source] = { items: [], maxQuality: 0, bestItem: null };
            }
            ...
            if (qualityResult.quality > grouped[source].maxQuality) {
                ...
                grouped[source].bestQualitySources = qualityResult.sources;
            }
        });
        ...
        sortedSources.forEach(function(source, index) {
            ...
            var sourceW = getSourceWeightByName(source);
            var voiceW = data.bestItem ? getVoiceWeightByName(data.bestItem.voice_name || data.bestItem.text || '') : 0;
            ...
            if (data.bestItem) {
                ...
                var bestVoiceW = getVoiceWeightByName(bestVoice);
                console.log('     🎤 Лучшая озвучка: "' + bestVoice + '" (вес ' + bestVoiceW + ')');
                console.log('     📝 Текст: "' + (data.bestItem.text || '') + '"');

                if (data.bestQualitySources && data.bestQualitySources.length > 0) {
                    console.log('     🔍 Качество найдено в:');
                    data.bestQualitySources.forEach(function(s) {
                        console.log('        - ' + s);
                    });
                }
            }
        ...
        if (bestSource) {
            ...
            console.log('');
            console.log('  📌 Рекомендация: переключитесь на источник "' + bestSource + '" для лучшего качества');
        } else {
    ...
    // ============================================================
    // ПОИСК И ПЕРЕКЛЮЧЕНИЕ НА ЛУЧШИЙ ИСТОЧНИК
    // ============================================================

    function findAndSwitchBestSource(movie) {
        debugLog('🎯 Поиск лучшего источника для:', movie.title || movie.name);

        getBalansersList(function(balansers) {
            debugLog('📋 Источники для проверки:', balansers);

            collectAllSources(movie, balansers, function(allItems) {
                // === ВЫВОД В КОНСОЛЬ ===
                logAnalysisResult(movie, allItems);

                if (allItems.length === 0) {
                    debugLog('❌ Нет данных для анализа');
                    return;
                }
                ...
                allItems.forEach(function(item) {
                    ...
                    var quality = getItemQuality(item).quality;
                    if (quality > grouped[source].maxQuality) {
                        grouped[source].maxQuality = quality;
                        grouped[source].bestItem = item;
                    }
                });
                ...
                if (!bestSource) {
                    debugLog('❌ Лучший источник не найден');
                    return;
                }

                debugLog('🏆 Лучший источник:', bestSource, '(качество ' + bestQuality + 'p, вес ' + bestTotalWeight + ')');
                ...
                if (Lampa.Noty && Lampa.Noty.show) {
                    Lampa.Noty.show('🔊 Выбран источник: ' + bestSource + ' (качество ' + bestQuality + 'p)');
                }
                ...
                debugLog('✅ Переключено на:', bestSource);
            });
        });
    }

    // ============================================================
    // КНОПКА "Лучший источник" В КАРТОЧКЕ
    // ============================================================
    ...
    // ============================================================
    // ПЕРЕХВАТ КОМПОНЕНТА (ОРИГИНАЛЬНАЯ ЛОГИКА)
    // ============================================================
    ...
    function installComponent() {
        ...
        debugLog('✅ Компонент установлен: ' + smartComponentName());
        return true;
    }
    ...
    function start() {
        ...
        window.smartonline_plugin = true;
        debugLog('🚀 Плагин SmartOnline запускается...');
        addLang();
        registerSettings();
        installPlayerHooks();
        installComponent();
        ...
        debugLog('✅ Плагин SmartOnline запущен');
    }
