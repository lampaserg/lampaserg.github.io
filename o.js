(function () {
  ...
  // КОНФИГУРАЦИЯ
  // ========================================================================
  var SERVER_URL = 'https://lampa.vip/';
  var PLUGIN_COMPONENT = 'lampa_vip';
  var PLUGIN_NAME = 'lampa.vip ⭐';
  ...
  function buildUrl(url, params) {
    ...
    if (url.indexOf('uid=') === -1) {
      url = Lampa.Utils.addUrlComponent(url, 'uid=' + encodeURIComponent(getUnicId()));
    }
    return url;
  }
  ...
  // ========================================================================
  // RCH (REMOTE COMMAND HANDLER)
  // ========================================================================
  var hostkey = SERVER_URL.replace(/^https?:\/\//, '').replace(/\/+$/, '');
  ...
  window.rch_nws[hostkey].typeInvoke = function rchtypeInvoke(host, call) {
    if (!window.rch_nws[hostkey].startTypeInvoke) {
      ...
      if (Lampa.Platform.is('android') || Lampa.Platform.is('tizen')) {
        ...
        net.silent(SERVER_URL.indexOf(location.host) >= 0 ? 'https://github.com/' : host + '/cors/check', function() {
          check(true);
        }, function() {
          check(false);
        }, false, { dataType: 'text' });
  ...
  window.rch_nws[hostkey].Registry = function RchRegistry(client, startConnection) {
    window.rch_nws[hostkey].typeInvoke(SERVER_URL, function() {
      ...
      client.on("RchClient", function(rchId, url, data, headers, returnHeaders) {
            ...
            new Response(compressedStream).arrayBuffer()
              .then(function(compressedBuffer) {
                ...
                if (compressedArray.length > html.length) {
                  ...
                  $.ajax({
                    url: SERVER_URL + 'rch/gzresult?id=' + rchId,
                    type: 'POST',
        ...
        if (url == 'eval') {
          ...
          console.log('RCH', url);
          network["native"](account(url), result, function() {
  ...
  function rchRun(json, call) {
    if (typeof NativeWsClient == 'undefined') {
      Lampa.Utils.putScript([SERVER_URL + 'js/nws-client-es5.js?v21042026'], function() {}, false, function() {
        rchInvoke(json, call);
      }, true);
    } else {
      rchInvoke(json, call);
  ...
  // ========================================================================
  // ОСНОВНОЙ КОМПОНЕНТ ДЛЯ ПОИСКА ОНЛАЙН-ВИДЕО
  // ========================================================================
  ...
  function component(object) {
    ...
    // Загрузка списка балансеров с поиском
    function ensureBalansersWithSearch() {
      ...
      balansers_with_search_promise = new Promise(function(resolve) {
        ...
        req.timeout(10000);
        req.silent(account(Defined.localhost + 'lite/withsearch'), function(json) {
    ...
    this.initialize = function() {
      ...
      this.externalids().then(function() {
        ...
        return ensureBalansersWithSearch().then(function(withSearch) {
          var hasSearch = false;
          for (var i = 0; i < withSearch.length; i++) {
            if (balanser.slice(0, withSearch[i].length) == withSearch[i]) {
              hasSearch = true;
              break;
            }
          }
    ...
    this.draw = function(items) {
      ...
      this.getEpisodes(items[0].season, function(episodes) {
        ...
        items.forEach(function(element, index) {
          ...
          if (serial && !episode) {
            image.append('<div class="online-prestige__episode-number">' + (element.episode || index + 1) + '</div>');
            loader.remove();
          ...
          left.forEach(function(episode) {
            ...
            if (episode.still_path) {
              ...
              img.onload = function() {
                image.addClass('online-prestige__img--loaded');
                loader.remove();
                image.append('<div class="online-prestige__episode-number">' + episode.episode_number + '</div>');
              };
    ...
    this.noConnectToServer = function(er) {
      ...
      html.find('.online-empty__time').text(er && er.accsdb ? er.msg : Lampa.Lang.translate('lampac_does_not_answer_text').replace('{balanser}', balanser));
      scroll.clear();
      scroll.append(html);
      this.loading(false);
    };
  ...
  // ========================================================================
  // ДОБАВЛЕНИЕ ИСТОЧНИКА В ГЛОБАЛЬНЫЙ ПОИСК
  // ========================================================================
  ...
  function startPlugin() {
    ...
    Lampa.Lang.add({
      ...
      lampac_balanser_dont_work: { ru: 'Поиск не дал результатов', uk: 'Пошук на ({balanser}) не дав результатів', en: 'Search on ({balanser}) did not return any results', zh: '搜索 ({balanser}) 未返回任何结果' },
    ...
    if (Lampa.Manifest.app_digital >= 177) {
      var balansers_sync = [
        ...
        "collaps", "hdvb", "kodik", "bamboo", "eneyida", "kinoukr", "uafilm", "uakino", "kinotochka",
        "remux", "anilibria", "animedia", "animego", "animevost", "animebesst", "alloha", "mirage",
        "phantom", "animelib", "moonanime", "vibix", "fancdn", "cdnvideohub", "vokino", "hydraflix",
        "videasy", "vidsrc", "movpi", "vidlink", "smashystream", "autoembed", "pidtor", "videoseed",
        "iptvonline", "veoveo", "kinoflix", "leproduction", "vkmovie", "kinogo", "kinobase", "asiage",
