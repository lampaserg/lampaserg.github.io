...
var ads = '<div style="padding: 0.3em 0.3em; padding-top: 0;"><div style="background: #3e3e3e; padding: 0.5em; border-radius: 1em;"><div style="line-height: 1.2;"><span style="color: #ffffff"><div style="text-align: center;">Мои плагины<br><span style="color: #f3d900">Сборник плагинов</span></span></div></div></div></div>';
...
// Функция для создания параметра с отображением ссылки
function createPluginParam(component, paramName, displayName, description, pluginUrl, author) {
    Lampa.SettingsApi.addParam({
        ...
        onChange: function(value) {
            if (value == '1') {
                itemON(pluginUrl, displayName, author, paramName);
            }
            if (value == '2') {
        ...
        onRender: function(item) {
            ...
            var pluginsArray = Lampa.Storage.get('plugins');
            setTimeout(function() {
                if ($('div[data-name="' + paramName + '"]').find('.settings-param__status').length === 0) {
                    $('div[data-name="' + paramName + '"]').append('<div class="settings-param__status one"></div>');
                }
                ...
                if (myResult && pluginStatus !== 0) {
                    $('div[data-name="' + paramName + '"]').find('.settings-param__status').removeClass('active error').addClass('active');
                } else if (pluginStatus === 0) {
                    $('div[data-name="' + paramName + '"]').find('.settings-param__status').removeClass('active error').css('background-color', 'rgb(255, 165, 0)');
                } else {
                    $('div[data-name="' + paramName + '"]').find('.settings-param__status').removeClass('active error').addClass('error');
                }
            }, 100);
...
Lampa.SettingsApi.addParam({
    ...
    param: { name: 'add_notice', type: 'title' },
    field: { name: notice }
});

createPluginParam('add_tmdb_proxy_plugin', 'ByLAMPA_TMDB', 'ByLAMPA TMDB Proxy', 'Проксирование постеров для сайта TMDB', 'https://bylampa.github.io/tmdb-proxy.js', '@bylampa');
createPluginParam('add_tmdb_proxy_plugin', 'Rootu_TMDB', 'Rootu TMDB Proxy', 'Проксирование постеров для сайта TMDB', 'http://plugin.rootu.top/tmdb.js', '@rootu');
createPluginParam('add_tmdb_proxy_plugin', 'Andrey_TMDB', 'Andrey TMDB Proxy', 'Проксирование постеров для сайта TMDB', 'http://185.209.28.172/tmdb', '@AndreyURL54');
...
createPluginParam('add_interface_plugin', 'Tricks', 'Приятные мелочи', 'Скринсейверы, часы в плеере и другое', 'https://andreyurl54.github.io/diesel5/tricks.js', '@AndreyURL54');
createPluginParam('add_interface_plugin', 'Rating', 'Рейтинг КиноПоиск и IMDB', 'Показ рейтинга КП и IMDB в карточке', 'https://bylampa.github.io/rating.js', '@t_anton');
createPluginParam('add_interface_plugin', 'Want', 'Старый стиль пунктов', 'Возвращает старый стиль отображения пунктов', 'http://github.freebie.tom.ru/want.js', '@VitalikPVA');
createPluginParam('add_interface_plugin', 'Sub_reset', 'Сброс настроек субтитров', 'Сбрасывает настройки субтитров по умолчанию', 'https://nb557.github.io/plugins/reset_subs.js', '@t_anton');
createPluginParam('add_interface_plugin', 'Mult', 'Мультфильмы', 'Заменяет пункт Аниме на Мульт', 'http://193.233.134.21/plugins/mult.js', '@AndreyURL54');
createPluginParam('add_interface_plugin', 'Collections', 'Коллекции', 'Коллекции фильмов и сериалов в главном меню', 'https://cub.red/plugin/collections', '@lampa');
createPluginParam('add_interface_plugin', 'Weather', 'Погода', 'Показывает время и погоду', 'https://bylampa.github.io/weather.js', '@scabrum');
createPluginParam('add_interface_plugin', 'Cub_off', 'Cub Off', 'Убирает элементы Cub Premium', 'https://bylampa.github.io/cub_off.js', '@scabrum');
createPluginParam('add_interface_plugin', 'Style_interface_fix', 'Стильный интерфейс', 'Новый стиль интерфейса', 'https://bylampa.github.io/interface.js', '@lampa');
createPluginParam('add_interface_plugin', 'New_cat', 'Дополнительные категории', 'Добавляет категории Документалки, Концерты, Мультфильмы', 'https://lampame.github.io/main/nc/nc.js', '@GwynnBleiidd');
createPluginParam('add_interface_plugin', 'New_source', 'Дополнительные источники', 'Дополнительные источники информации о фильмах', 'https://bylampa.github.io/source.js', '@scabrum');
createPluginParam('add_interface_plugin', 'kp_source', 'Источник КП', 'Добавляет источник КиноПоиск', 'https://bylampa.github.io/kp_source.js', '@bylampa');
createPluginParam('add_interface_plugin', 'Start', 'Start', 'Позволяет заходить на заблокированные карточки', 'https://bylampa.github.io/start.js', '@scabrum');
createPluginParam('add_interface_plugin', 'goldtheme', 'Золотая тема', 'Включает золотую тему', 'https://bazzzilius.github.io/scripts/gold_theme.js', '@BazZziliuS');
createPluginParam('add_interface_plugin', 'concert_search', 'Поиск концертов', 'Поиск концертов через парсер Jackett', 'https://lampame.github.io/main/cts.js', '@GwynnBleiidd');
createPluginParam('add_interface_plugin', 'Rezka_comments', 'Комментарии от Rezka', 'Выводит комментарии с Rezka', 'https://BDVBurik.github.io/rezkacomment.js', '@BDV_Burik');
createPluginParam('add_interface_plugin', 'Shikimori', 'LME Shikimori', 'Информация по аниме с Shikimori', 'https://lampame.github.io/main/shikimori.js', '@GwynnBleiidd');
createPluginParam('add_interface_plugin', 'ts_del', 'Remove TS', 'Убирает карточки с качеством TS', 'http://193.233.134.21/plugins/nots', '@AndreyURL54');
createPluginParam('add_interface_plugin', 'rus_movie', 'Русские новинки', 'Подборка русских новинок', 'https://bylampa.github.io/rus_movie.js', '@bylampa');
createPluginParam('add_interface_plugin', 'in_qual', 'В качестве', 'Фильмы в высоком качестве', 'https://bylampa.github.io/in_quality.js', '@bylampa');
createPluginParam('add_interface_plugin', 'snow', 'Снег', 'Анимация падающего снега', 'https://bylampa.github.io/snow.js', '@bylampa');
createPluginParam('add_interface_plugin', 'logo_title', 'Лого вместо названия', 'Замена названия на логотип', 'https://bylampa.github.io/logo_title.js', '@bylampa');
createPluginParam('add_interface_plugin', 'trakt', 'TraktTV', 'Интеграция с Trakt.TV', 'https://lampame.github.io/main/trakttv.js', '@lme');
createPluginParam('add_interface_plugin', 'head_filter', 'Настройка шапки', 'Скрытие элементов в шапке', 'https://and7ey.github.io/lampa/head_filter.js', '@and7ey');
createPluginParam('add_interface_plugin', 'cardify', 'Cardify', 'Обновленный вид карточек', 'https://bylampa.github.io/cardify.js', '@lampa');
createPluginParam('add_interface_plugin', 'back_menu', 'Меню выход', 'Кастомное меню выхода', 'https://bylampa.github.io/backmenu.js', '@bylampa');
createPluginParam('add_interface_plugin', 'my_themes', 'Мои темы', 'Пользовательские темы', 'https://bylampa.github.io/themes.js', '@bylampa');
createPluginParam('add_interface_plugin', 'inter_movie', 'Зарубежные подборки', 'Подборки зарубежных фильмов', 'https://bylampa.github.io/inter_movie.js', '@bylampa');
createPluginParam('add_interface_plugin', 'rate_lampa', 'Рейтинг Lampa', 'Рейтинг на основе оценок пользователей', 'https://bylampa.github.io/rate_lampa.js', '@AndreyURL54');
createPluginParam('add_interface_plugin', 'old_cards_status', 'Статус на старой карточке', 'Статус на карточке со старым стилем', 'https://bylampa.github.io/old_card_status.js', '@bylampa');
createPluginParam('add_interface_plugin', 'eps_and_seas', 'Состояние сериала', 'Отображает текущее состояние сериала', 'https://bylampa.github.io/seas_and_eps.js', '@bylampa');
createPluginParam('add_interface_plugin', 'anime_tmdb', 'Аниме', 'Подборки аниме от TMDB', 'https://bylampa.github.io/anime.js', '@bylampa');
createPluginParam('add_interface_plugin', 'my_bookmarks', 'Мои закладки', 'Создание папок в закладках', 'https://bylampa.github.io/my_bookmarks.js', '@bylampa');
createPluginParam('add_interface_plugin', 'color_vote', 'Цветные оценки', 'Окрашивает оценки в зависимости от значения', 'https://bylampa.github.io/color_vote.js', '@fovway');
createPluginParam('add_interface_plugin', 'tr_itunes', 'Трейлеры Itunes', 'Трейлеры с Itunes', 'https://plugin.rootu.top/trailers.js', '@rootu');
createPluginParam('add_interface_plugin', 'tr_rutube', 'Трейлеры Rutube', 'Трейлеры с Rutube', 'https://plugin.rootu.top/rutube.js', '@rootu');
createPluginParam('add_interface_plugin', 'surs', 'Aviamovie Surs', 'Уникальные подборки на главной странице', 'https://aviamovie.github.io/surs.js', '@pilot_valliko');
createPluginParam('add_interface_plugin', 'size', 'Фиксированный размер', 'Меняет размеры интерфейса', 'https://nb557.github.io/plugins/fix_size.js', '@t_anton');
createPluginParam('add_interface_plugin', 'stat', 'Статистика', 'Ведение статистики просмотров', 'https://and7ey.github.io/lampa/stats.js', '@alukyanov');
createPluginParam('add_interface_plugin', 'lable_tv', 'Лейбл сериала', 'Заменяет TV на Сериал', 'https://bylampa.github.io/lable_serial.js', '@bylampa');
createPluginParam('add_interface_plugin', 'full_center', 'Card elems center', 'Центрирует элементы в карточке', 'https://bylampa.github.io/full_center.js', '@bylampa');
createPluginParam('add_interface_plugin', 'bylampa_source', 'Источник ByLAMPA', 'Кастомный источник контента', 'https://bylampa.github.io/bylampa_source.js', '@bylampa');
createPluginParam('add_interface_plugin', 'trailers_on_main', 'Трейлеры', 'Добавляет пункт Трейлеры на главную', 'https://mastermagic98.github.io/l_plugins/upcoming.js', '@myroslav_kuzyshyn');
createPluginParam('add_interface_plugin', 'tr_off', 'Trailer off', 'Убирает трейлеры в карточке', 'https://bylampa.github.io/trailer_off.js', '@bylampa');
createPluginParam('add_interface_plugin', 'quality', 'Quality', 'Отметки качества на карточках', 'https://bylampa.github.io/quality.js', '@bylampa');
createPluginParam('add_interface_plugin', 'filter_content', 'Фильтр контента', 'Фильтрация вывода карточек', 'https://bylampa.github.io/filter_content.js', '@bylampa');
createPluginParam('add_interface_plugin', 'rate_on_main', 'Источник рейтинга', 'Меняет источник рейтинга на главной', 'https://bylampa.github.io/rate_on_main.js', '@bylampa');
createPluginParam('add_interface_plugin', 'orig_title', 'Оригинальное название', 'Показывает оригинальное название', 'https://bylampa.github.io/orig_title.js', '@bylampa');
createPluginParam('add_interface_plugin', 'animated_reaction', 'Анимированные реакции', 'Анимирует реакции от CUB', 'https://bylampa.github.io/animated_reaction.js', '@AndreyURL54');
createPluginParam('add_interface_plugin', 'time2end', 'Time2end', 'Время окончания фильма в плеере', 'https://bylampa.github.io/time2end.js', '@AndreyURL54');
createPluginParam('add_interface_plugin', 'top', 'Top ByLAMPA', 'Топ контента от ByLAMPA', 'https://bylampa.github.io/top.js', '@bylampa');
...
createPluginParam('add_management_plugin', 'Exit_Menu', 'Выход', 'Добавляет пункт Выход в главное меню', 'https://tsynik.github.io/lampa/e.js', '@tsynik');
createPluginParam('add_management_plugin', 'Hot_Buttons', 'Горячие кнопки', 'Управление плеером с пульта', 'https://nnmdd.github.io/lampa_hotkeys/hotkeys.js', '@nnmd');
createPluginParam('add_management_plugin', 'DLNA', 'DLNA (Tizen, Orsay)', 'DLNA для устройств', 'https://bylampa.github.io/dlna.js', '@lampa');
createPluginParam('add_management_plugin', 'Select_Weapon', 'Тип управления', 'Выбор типа управления', 'https://nemiroff.github.io/lampa/select_weapon.js', '@nemiroff');
createPluginParam('add_management_plugin', 'Touch_off', 'Выключение тача', 'Отключает сенсорное управление', 'https://nb557.github.io/plugins/not_mobile.js', '@t_anton');
createPluginParam('add_management_plugin', 'Wsoff', 'Wsoff', 'Отключение ошибки WebSocket', 'http://plugin.rootu.top/wsoff.js', '@rootu');
createPluginParam('add_management_plugin', 'Redirect', 'Смена сервера', 'Смена сервера приложения', 'https://bylampa.github.io/redirect.js', '@scabrum');
createPluginParam('add_management_plugin', 'setprotect', 'Доступ к настройкам', 'Закрывает доступ к настройкам паролем', 'http://193.233.134.21/plugins/setprotect', '@AndreyURL54');
createPluginParam('add_management_plugin', 'Sort_mainmenu', 'Сортировка главного меню', 'Редактирование главного меню', 'http://193.233.134.21/plugins/menusort', '@AndreyURL54');
createPluginParam('add_management_plugin', 'bind_but', 'Бинд кнопок', 'Управление цветными кнопками пульта', 'https://apxubatop.github.io/lmpPlugs/tvbuttontst.js', '@Juri_Z');
createPluginParam('add_management_plugin', 'infuse_save', 'Infuse save', 'Сохранение торрентов в Infuse', 'https://lampame.github.io/main/infuseSave.js', '@lme');
createPluginParam('add_management_plugin', 'cub_sync', 'CUB Sync', 'Синхронизация с CUB', 'https://bylampa.github.io/cub_sync.js', '@levende');
createPluginParam('add_management_plugin', 'Reload', 'Перезагрузка приложения', 'Кнопка перезагрузки', 'https://bylampa.github.io/reload.js', '@bylampa');
createPluginParam('add_management_plugin', 'sort_main_menu', 'Sort main menu', 'Сортировка главного меню', 'https://bylampa.github.io/sort_main_menu.js', '@bylampa');
createPluginParam('add_management_plugin', 'speedtest', 'Speedtest', 'Измерение скорости интернета', 'https://bylampa.github.io/speedtest.js', '@AndreyURL54');
...
createPluginParam('add_online_plugin', 'Online_Mod', 'Online Mod', 'Онлайн просмотр с балансерами', 'https://bylampa.github.io/online_mod.js', '@t_anton');
createPluginParam('add_online_plugin', 'Онлайн_Prestige', 'Онлайн Filmix', 'Онлайн просмотр с Filmix', 'https://bylampa.github.io/fx.js', '@rik');
createPluginParam('add_online_plugin', 'online_cinema', 'Онлайн Cinema', 'Онлайн просмотр', 'https://bylampa.github.io/cinema.js', '@cinema');
createPluginParam('add_online_plugin', 'Showy_WTCH', 'Showy WTCH (free)', 'Онлайн просмотр', 'http://wtch.ch/m', '@showy');
createPluginParam('add_online_plugin', 'smotret_4k', 'Смотреть 4K', 'Онлайн просмотр в 4K', 'http://smotretk.com/online.js', '@showy');
...
createPluginParam('add_torrent_plugin', 'Switch_Parser', 'Переключение парсеров', 'Переключение между парсерами Jackett', 'https://bylampa.github.io/jackett.js', '@AndreyURL54');
createPluginParam('add_torrent_plugin', 'Tracks', 'Tracks', 'Замена названий аудиодорожек', 'https://bylampa.github.io/tracks.js', '@lampa');
createPluginParam('add_torrent_plugin', 'Setting_torrents', 'Настройка торрентов', 'Для Web OS и Tizen', 'https://bylampa.github.io/etor.js', '@lampa');
createPluginParam('add_torrent_plugin', 'Check_server', 'Поиск локального TorrServera', 'Поиск TorrServer', 'http://193.233.134.21/plugins/checker.js', '@AndreyURL54');
createPluginParam('add_torrent_plugin', 'Torr_download', 'Закачка торрентов', 'Загрузка через qBittorent/Transmission', 'https://lampame.github.io/main/torrentmanager/torrentmanager.js', '@feliks');
createPluginParam('add_torrent_plugin', 'second_but_torrents', 'Вторая кнопка Торренты', 'Добавляет вторую кнопку Торренты', 'https://github.freebie.tom.ru/torrents.js', '@VitalikPVA');
createPluginParam('add_torrent_plugin', 'free_torr', 'Free Torrserver', 'Автоподстановка Torrserver', 'https://bylampa.github.io/freetorr.js', '@scabrum');
createPluginParam('add_torrent_plugin', 'visual_ts', 'Визуализация загрузки TS', 'Визуализация загрузки TorrServer', 'https://plugin.rootu.top/ts-preload.js', '@rootu');
...
createPluginParam('add_tv_plugin', 'Diesel', 'Дизель ТВ', 'Бесплатный просмотр телеканалов', 'https://andreyurl54.github.io/diesel5/diesel.js', '@AndreyURL54');
createPluginParam('add_tv_plugin', 'Kulik', 'Kulik', 'IPTV каналы', 'http://cdn.kulik.uz/cors', '@SawamuraRen');
createPluginParam('add_tv_plugin', 'IPTV', 'IPTV', 'IPTV каналы', 'http://cub.red/plugin/iptv', '@lampa');
createPluginParam('add_tv_plugin', 'Hack_TV', 'Hack TV', 'IPTV каналы', 'https://bylampa.github.io/tv.js', '@scabrum');
...
createPluginParam('add_radio_plugin', 'Record', 'Радио Record', 'Прослушивание радио Radio Record', 'http://cub.red/plugin/radio', '@lampa');
createPluginParam('add_radio_plugin', 'Record_Mod', 'Радио Record Mod', 'Radio Record с единым списком', 'https://lampame.github.io/main/rradio.js', '@GwynnBleiidd');
createPluginParam('add_radio_plugin', 'Radio_Soma', 'Радио SomaFM', 'Уникальные каналы радио', 'https://tsynik.github.io/lampa/soma.js', '@tsynik');
...
createPluginParam('add_sisi_plugin', 'sisi_durex', 'Клубничка', 'Плагин для просмотра контента 18+', 'https://bylampa.github.io/sisi.js', '@rik');
